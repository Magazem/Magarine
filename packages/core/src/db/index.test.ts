...
test('a database migrated only to 0001 picks up 0002 (usage_json) on next open, without re-running 0001', () => {
  ...
  try {
    ...
    assert.equal(JSON.parse(row.usage_json).inputTokens, 1);
    db.close();
  } finally {
