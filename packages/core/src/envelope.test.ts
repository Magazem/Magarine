...
function ticket(overrides: Partial<TicketEnvelope>): TicketEnvelope {
  return {
    ...
    completedDependencies: [{ ticketId: 'tkt_dep', title: 'Dep ticket', summary: 'Dep summary text' }],
...
test('prompt for one ticket contains nothing from an unrelated ticket', () => {
  const a = ticket({
    ...
    completedDependencies: [{ ticketId: 'dep', title: 'A-dep-AAA444', summary: 'A-dep-summary-AAA555' }],
  });
  const b = ticket({
    title: 'Ticket B unique marker BBB111',
    description: 'Description B unique marker BBB222',
    acceptanceCriteria: ['B-criterion-BBB333'],
    completedDependencies: [{ ticketId: 'dep2', title: 'B-dep-BBB444', summary: 'B-dep-summary-BBB555' }],
  });
